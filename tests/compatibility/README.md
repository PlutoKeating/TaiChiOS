...
The smoke first proves that Bubblewrap can create the offline namespace. On locked-down CI hosts that deny unprivileged user namespaces, it retries only the Bubblewrap boundary through passwordless `sudo`; the runtime itself remains inside the same network-isolated sandbox. Failures include the captured pseudo-terminal transcript.
Smoke 会先验证 Bubblewrap 确实能够创建断网命名空间。若 CI 宿主禁止非特权 user namespace，则仅通过免密 `sudo` 重试 Bubblewrap 边界；运行时仍处于相同的网络隔离沙箱中。失败信息会附带捕获的伪终端记录。
